...
- Keep `~/AGENTS.md` for global rules/context only, so cross-project guidance stays in one stable place.
...
- Keep `README.md` current whenever behavior or status changes, so humans and agents see the same reality.
